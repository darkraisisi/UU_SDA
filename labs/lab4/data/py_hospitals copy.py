...
destination_expr = QgsExpression(f'destination_id=125')
...
for f in matrix.getFeatures(QgsFeatureRequest(destination_expr)):
    ...
    origin_coords =  [(f.geometry().asPoint().x(), f.geometry().asPoint().y()) for f in origin_feature]
    print(origin_coords)
    destination_feature = destination_layer.getFeatures(QgsFeatureRequest(destination_expr))
    destination_coords =  [(f.geometry().asPoint().x(), f.geometry().asPoint().y()) for f in destination_feature]
    params = {
        'INPUT':'/home/daviddemmers/Documents/uu/SDA/labs/lab4/data/Lab4data/roads_ams_2008.shp',
