...
rates = [x / 1000 for x in range(10, 20, 1)] 
print(len(rates), rates)
errors = numpy.zeros(len(rates))
for i, rate in enumerate(rates):
    # meltRateParameter = 0.003
    meltRateParameter = rate
    ...
    SS = numpy.mean((streamFlowModelled[(365*2):] - streamFlowObserved[(365*2):])**2.0)
    errors[i] = SS
    print('meltrate parameter is:', meltRateParameter, 'sum of squares is ', SS)
    
t = numpy.row_stack((rates, errors))
numpy.savetxt('bruteforce_last.txt', t)
